...
case class ZookeeperConfig(root: Path, host: String, port: Int, timeout: FiniteDuration, id: String @@ InstanceId)
...
object ZookeeperConfig {
  ...
  def create[F[_]: UUIDGen: Functor](
    ...
    UUIDGen[F].randomUUID.map(id => ZookeeperConfig(root, host, port, timeout, id.toString.taggedWith[InstanceId]))
